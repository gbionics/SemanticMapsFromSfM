...
class Config:
    ...
    # Expected format: same as train_lerf/train_scannet - masks_dir contains:
    #   - {image_name}.npy: segmentation mask (H, W)
    ...
    seg_opt_lr: float = 2.5e-3
    # Regularization for segmentation feature optimization
    seg_opt_reg: float = 1e-6
    # Weight for contrastive loss (lambda_1)
    contrastive_lambda_1: float = 1e-4
    # Weight for contrastive loss (lambda_2)
    contrastive_lambda_2: float = 1e-2
    # Epsilon for contrastive loss
    contrastive_epsilon: float = 100
...
def create_splats_with_optimizers(
    ...
    seg_features_dim: Optional[int] = None,
    device: str = "cuda",
    ...
    # Add segmentation features if enabled
    if seg_features_dim is not None:
        seg_features = torch.rand(N, seg_features_dim)  # [N, seg_features_dim]
        params.append(("seg_features", torch.nn.Parameter(seg_features), 2.5e-3))
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        self.valset = Dataset(self.parser, split="val", masks_dir=cfg.masks_dir)
        self.scene_scale = self.parser.scene_scale * 1.1 * cfg.global_scale
        print("Scene scale:", self.scene_scale)

        # Model
        feature_dim = 32 if cfg.app_opt else None
        seg_features_dim = cfg.seg_features_dim if cfg.seg_opt else None
        self.splats, self.optimizers = create_splats_with_optimizers(
            ...
            seg_features_dim=seg_features_dim,
            device=self.device,
        )
        ...
        self.seg_optimizers = []
        if cfg.seg_opt and "seg_features" in self.splats:
            self.seg_optimizers = [
                torch.optim.Adam(
                    [self.splats["seg_features"]],
                    lr=cfg.seg_opt_lr * math.sqrt(cfg.batch_size),
                    weight_decay=cfg.seg_opt_reg,
                )
            ]
    ...
    def rasterize_splats(
        ...
        height: int,
        **kwargs,
    ) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Dict, Optional[Tensor]]:
        ...
        # Render segmentation features if enabled
        render_seg_features = None
        if self.cfg.seg_opt and "seg_features" in self.splats:
            seg_features = self.splats["seg_features"]  # [N, seg_features_dim]
            # Render segmentation features - we need to rasterize with seg_features as "colors"
            # We'll normalize seg_features to [0, 1] range for rasterization
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            seg_loss = 0.0
            if cfg.seg_opt and render_seg_features is not None and step >= 10_000:
                if "segmask" in data:
                    # Use actual segmentation masks from the dataset
                    segmask = data["segmask"].to(device).squeeze(0)  # [H, W]
                    seg_features_map = render_seg_features.squeeze(0)  # [H, W, D]
                    
                    # Compute unique segment IDs and their counts
                    unique_ids = torch.unique(segmask)
                    if len(unique_ids) > 1:
                        n_i_list = torch.tensor(
                            [torch.sum(segmask == uid).float() for uid in unique_ids],
                            device=device
                        )
                        
                        # Use hierarchical contrastive loss with adjacency/Laplacian matrices if available
                        if "adj_mtx" in data and "lap_mtx" in data:
                            # Full hierarchical loss with graph-based components
                            seg_loss = hierarchical_contrastive_loss(
                                segmask=segmask,
                                features=seg_features_map,
                                adj_mtx=data["adj_mtx"].to(device),
                                lap_mtx=data["lap_mtx"].to(device),
                                id_unique_list=unique_ids,
                                n_i_list=n_i_list,
                                dim_features=cfg.seg_features_dim,
                                lambda_1_val=cfg.contrastive_lambda_1,
                                lambda_2_val=cfg.contrastive_lambda_2,
                                epsilon=cfg.contrastive_epsilon,
                            )
                        else:
                            # Simple contrastive loss without graph components
                            seg_loss = hierarchical_contrastive_loss(
                                segmask=segmask,
                                features=seg_features_map,
                                id_unique_list=unique_ids,
                    ...
                    # Use simplified loss that encourages feature decorrelation
                    render_seg_features_squeezed = render_seg_features.squeeze(0).permute(2, 0, 1)  # [D, H, W]
                    # Simple regularization: encourage decorrelated features
                    feature_cov = torch.mm(
                        render_seg_features.reshape(-1, cfg.seg_features_dim).t(),
                        render_seg_features.reshape(-1, cfg.seg_features_dim)
                    )
                    feature_cov = feature_cov / (render_seg_features.shape[1] * render_seg_features.shape[2])
                    
                    # Encourage orthogonality of features
                    eye_matrix = torch.eye(cfg.seg_features_dim, device=device)
                    seg_loss = torch.norm(feature_cov - eye_matrix) ** 2
                
                loss += seg_loss * (cfg.contrastive_lambda_1 + cfg.contrastive_lambda_2)
            ...
            if cfg.seg_opt and seg_loss > 0:
                desc += f" seg loss={seg_loss:.6f}"
            ...
            for optimizer in self.app_optimizers:
                optimizer.step()
                optimizer.zero_grad(set_to_none=True)
            for optimizer in self.seg_optimizers:
                optimizer.step()
                optimizer.zero_grad(set_to_none=True)
    ...
    def render_traj(self, step: int):
        ...
        for i in tqdm.trange(len(camtoworlds), desc="Rendering trajectory"):
            renders, _, _, surf_normals, _, _, _, render_seg_features = self.rasterize_splats(
            ...
            # Prepare PCA-based feature-field visualization if available
            if render_seg_features is not None:
                feat_map = render_seg_features.squeeze(0)  # [H, W, D]
                Hf, Wf, D = feat_map.shape
                # If dimensionality is small, pad to at least 3 dims for stability
                if D < 3:
                    pad = torch.zeros((Hf, Wf, 3 - D), device=feat_map.device)
                    feat_pad = torch.cat([feat_map, pad], dim=-1)
                    X = feat_pad.reshape(-1, 3)  # [N, 3]
                else:
                    # Center features and run PCA via SVD on CPU for stability
                    X = feat_map.reshape(-1, D)  # [N, D]
                    # subtract mean
                    X_mean = torch.mean(X, dim=0, keepdim=True)
                    Xc = X - X_mean
                    try:
                        # compute SVD on CPU to avoid GPU numeric issues for large D
                        U, S, Vh = torch.linalg.svd(Xc.cpu(), full_matrices=False)
                        comps = Vh[:3, :].to(feat_map.device)  # [3, D]
                        proj = (Xc.to(feat_map.device) @ comps.t())  # [N, 3]
                        X = proj
                    except Exception:
                        # fallback: use first 3 dims
                        X = X[:, :3]

                # Reshape back to H,W,3
                feat_rgb = X.reshape(Hf, Wf, 3)

                # Normalize per-channel to [0,1]
                fmin = feat_rgb.reshape(-1, 3).min(dim=0).values
                fmax = feat_rgb.reshape(-1, 3).max(dim=0).values
                denom = (fmax - fmin)
                # avoid division by zero
                denom[denom == 0] = 1.0
                feat_rgb = (feat_rgb - fmin[None, None, :]) / denom[None, None, :]
                feat_rgb = torch.clamp(feat_rgb, 0.0, 1.0)
            else:
                feat_rgb = torch.zeros_like(colors)

            # write images (image | depth | feature)
            canvas = torch.cat(
                [colors, depths.repeat(1, 1, 3), feat_rgb], dim=0 if width > height else 1
            )
